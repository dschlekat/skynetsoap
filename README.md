...
The core functionality of the package resides within the soap class, which is initialized with the observation ID and the coordinates of the object. The user can then call the aperture photometry method to conduct the photometry, and the plot method to create lightcurves in a variety of units (flux, relative flux, magnitude). The user can also call the table method to create a photometric table or a GCN formatted table. Forced photometry on a particular point is also available, as is a method to determine the limiting magnitude of each image (see ImageDepth class). Finally, a special relative photometric pipeline is available for the observation of stars with small, but detectable, periodic variability (see RelativePhotometry class). 
